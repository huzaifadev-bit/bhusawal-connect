// Bhusawal Connect Google Maps Live Delivery Tracking Engine
(function(window) {
  class GoogleLiveTracker {
    ...
    initMap() {
      ...
      if (window.google && window.google.maps) {
        // Initialize real Google Maps instance
        this.map = new google.maps.Map(container, {
          center: { lat: 21.0487, lng: 75.7919 },
          zoom: 15,
        ...
        // Rider Marker (Custom SVG Scooter with Bearing Rotation)
        const scooterSvg = `
        ...
        // Fallback Leaflet engine with Google Maps Tiles format
    ...
    updateRiderLocation(newLat, newLng, heading = 0) {
      const oldLat = this.riderPos.lat;
      const oldLng = this.riderPos.lng;
      this.riderPos = { lat: newLat, lng: newLng, heading };

      if (window.google && window.google.maps && this.riderMarker instanceof google.maps.Marker) {
        this.riderMarker.setPosition(new google.maps.LatLng(newLat, newLng));
      } else if (this.riderMarker && typeof this.riderMarker.setLatLng === 'function') {
        this.riderMarker.setLatLng([newLat, newLng]);
      }

      // Calculate distance to destination in meters
      const distMeters = this.getHaversineMeters(newLat, newLng, this.custPos.lat, this.custPos.lng);

      // Trigger Proximity Alerts at 500m, 300m, 100m, 0m
      if (distMeters <= 500 && this.lastNotifiedDist > 500) {
        this.onNotification('500M_NEARBY', 'Rider is approaching your neighborhood (< 500m away)');
      }
      ...
      // Recalculate Google Maps Route & Traffic ETA
      this.calculateRoute();
    }
